...
class Torrentland:
    ...
    def parse_event(event: events.NewMessage.Event) -> TorrentData:
        ...
        if data["base_url"] and data["torrent_name"]:
            # Allows to link the torrent name to the torrent page in Telegram messages
            data["torrent_name"] = f"[{data['torrent_name']}]({data['base_url']})"
